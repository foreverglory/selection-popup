...
export default {
  components: {
    "selection-popup-icon": Icon,
    "selection-popup-dialog": Dialog
  },
  computed: {
    selected: {
      get: function() {
  ...
  watch: {
    init(newVal) {
      if (newVal) {
        let vm = this;
        let { key, mouse } = this.$store.state.activate;
        let selectionEvent;
        if (key && key !== "0") {
  ...
  methods: {
    ...
    onSelection(event) {
      ...
      if (word) {
        ...
        var direction = [];
        if (
          event.screenY + boxHeight > document.documentElement.clientHeight &&
          boxHeight < document.documentElement.clientHeight / 2
        ) {
          //向上
          vm.$set(vm, "style", {
            left: event.pageX + "px",
